...
export default function CategoriesPage() {
  ...
  async function load() {
    try {
      ...
      const json = (await res.json()) as ApiList<Category[]>
  ...
  async function onDelete(id: string) {
    ...
    const json = (await res.json()) as ApiList<null>
  ...
  return (
    <div className="mx-auto max-w-3xl p-6 space-y-8">
      ...
      <div className="rounded-2xl border p-4">
        <h2 className="mb-3 font-medium">Nueva categoría</h2>
        <CategoryForm onSubmit={onCreate} />
      </div>

      <div className="rounded-2xl border p-4">
        <h2 className="mb-3 font-medium">Tus categorías</h2>
        <CategoriesTable
          loading={loading}
